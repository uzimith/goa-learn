...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "show",
		Short: `Get arcitle by id`,
	}
	tmp2 := new(ShowArticleCommand)
	sub = &cobra.Command{
		Use:   `article ["/article/ID"]`,
		Short: ``,
...
func (cmd *ListArticleCommand) Run(c *client.Client, args []string) error {
	...
	resp, err := c.ListArticle(ctx, path, cmd.ID)
	if err != nil {
...
func (cmd *ListArticleCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var id []int
	cc.Flags().IntSliceVar(&cmd.ID, "id", id, `Filter by ids`)
}
...
func (cmd *ShowArticleCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/article/%v", cmd.ID)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
